...
async def test_create_project_port_endpoint_contract() -> None:
    created = make_port("p1", "port-1", from_port=80, port_number=6, protocol="tcp")
    ...
    assert response.data.id == "port-1"
...
async def test_list_project_ports_endpoint_contract() -> None:
    ...
    assert response.data.items[0].id == "port-1"
    assert usecase.calls == [
        ((), {"project_id": "p1", "user_id": "u1", "limit": 20, "cursor": "port-0"})
    ]


async def test_update_project_port_endpoint_contract() -> None:
    updated = make_port("p1", "port-1", from_ip="10.0.0.0/24", from_port=8080, port_number=6, protocol="tcp")
    ...
    assert usecase.calls == [
        ((), {"project_id": "p1", "port_id": "port-1", "request": payload, "user_id": "u1"})
    ]


async def test_delete_project_port_endpoint_contract() -> None:
    deleted = make_port("p1", "port-1", from_port=80, port_number=6, protocol="tcp")
    ...
    assert response.data.id == "port-1"
    assert usecase.calls == [
        ((), {"project_id": "p1", "port_id": "port-1", "user_id": "u1"})
    ]
