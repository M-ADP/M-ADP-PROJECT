...
async def test_update_project_name_endpoint_contract() -> None:
    updated = make_project(1, user_id=1, name="beta")
    ...
    assert usecase.calls == [((1, payload), {"user_id": 1})]


async def test_delete_project_endpoint_contract() -> None:
    deleted = make_project(1, user_id=1, name="alpha")
...
async def test_list_project_members_endpoint_contract() -> None:
    ...
    assert response.data.items[0].user_id == 1
    assert usecase.calls == [
        ((), {"project_id": 1, "user_id": 1, "limit": 20, "cursor": "m0"})
    ]
...
async def test_remove_project_member_endpoint_contract() -> None:
    ...
    assert response.data.user_id == 2
    assert usecase.calls == [
        ((), {"project_id": 1, "target_user_id": 2, "user_id": 1})
    ]
...
async def test_transfer_project_ownership_endpoint_contract() -> None:
    ...
    payload = ProjectOwnerTransfer(target_user_id=2)
