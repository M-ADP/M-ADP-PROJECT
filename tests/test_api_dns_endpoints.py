...
async def test_create_dns_endpoint_contract() -> None:
    created = make_dns("p1", "dns-1", dns_name="app.mdeveloper.platform", state=DNSState.PENDING)
...
async def test_get_dns_endpoint_contract_with_data() -> None:
    found = make_dns("p1", "dns-1", dns_name="app.mdeveloper.platform", state=DNSState.ACTIVE)
...
async def test_delete_dns_endpoint_contract() -> None:
    deleted = make_dns("p1", "dns-1", dns_name="app.mdeveloper.platform", state=DNSState.ACTIVE)
    ...
    assert response.data.id == "dns-1"
    assert usecase.calls == [(("p1", "dns-1"), {"user_id": "u1"})]


async def test_update_dns_endpoint_contract() -> None:
    updated = make_dns("p1", "dns-1", dns_name="new.mdeveloper.platform", state=DNSState.ACTIVE)
    ...
    assert usecase.calls == [(("p1", "dns-1", payload), {"user_id": "u1"})]
...
async def test_bind_port_to_dns_endpoint_contract() -> None:
    ...
    payload = DNSPortBinding(port_id="port-1")
    ...
    assert response.data.port_id == "port-1"
    assert usecase.calls == [(("p1", "dns-1", payload), {"user_id": "u1"})]
